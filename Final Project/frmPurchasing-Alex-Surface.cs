...
namespace Final_Project
{
    public partial class frmPurchasing : Form
    {
        ...
        private void btnPurchase_Click(object sender, EventArgs e)
        {
            ...
            accessDatabase(cuID, holder); //Keeping it in 'holder' to add it on next SQL command
            string customerID = holder.Rows[0][0].ToString();
            holder.Clear();

            if (rdoFirstClass.Checked == true)
            {
                writeDatabase("INSERT INTO Orders (FlightID, CustomerID, CustomerLastName, CustomerFirstName, CreditCard, Destination, DestinationSpace, FirstClassSeats, DepartureDate) VALUES ('" + vt.Rows[0][0] + "', '"+ customerID +  "','" + lName + "', '" + fName + "', '" + creditCard + "', '" + vt.Rows[0][3].ToString() + "', '" + vt.Rows[0][4].ToString() + "', " + cmbSeats.Text + ", '" + vt.Rows[0][5].ToString() + "')");
            ...
            {
                writeDatabase("INSERT INTO Orders (FlightID, CustomerID, CustomerLastName, CustomerFirstName, CreditCard, Email, Destination, DestinationSpace, CoachSeats, DepartureDate) VALUES (" + vt.Rows[0][0] + ", '" + customerID + "','" + lName + "', '" + fName + "', '" + creditCard + "', '"+eMail+"', '" + vt.Rows[0][3].ToString() + "', '" + vt.Rows[0][4].ToString() + "', " + cmbSeats.Text + ", '" + vt.Rows[0][5].ToString() + "')");
